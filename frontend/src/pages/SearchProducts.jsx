...
const SearchProducts = () => {
    ...
    const { searchproducts,searchnumOfPr,searchPage } = useSelector(s => s.Products)
    ...
    useEffect(() => {
        query && dispatch(searchProducts({ query, page: 1 }))
    }, [query])

    return (
        <div>  {searchproducts?.map(product => <div key={Math.random()} className=' inline-block  w-full  max-w-[400px] '>
            <div className="relative m-1 flex w-full scale-[85%] max-w-[340px] mx-auto flex-col overflow-hidden rounded-lg border dark:border-slate-700 border-gray-100 dark:bg-slate-800 dark:text-white bg-white shadow-md">
                ...
                <div className="mt-4 px-5 pb-5">
                    ...
                    <div className=' flex items-center justify-between '>
                        ...
                        <div
                            onClick={() => { user?.cart?.some(pr => pr.product == product._id) ? navigate('/cart') : navigate(`/product/${product._id}`) }}
                        ...
                        <div
                            onClick={() => user ? dispatch(addProductToWhishlist({ productID: product._id })) : navigate('/login')}
        ...
        {new Array(searchnumOfPr && Math.ceil(searchnumOfPr / 10)).fill('') > 1 && <div
          onClick={() => dispatch(getProducts({query, page: searchPage - 1 }))}
        ...
        <div className="items-center mx-auto flex gap-x-3">
          ...
          {new Array(searchnumOfPr && Math.ceil(searchnumOfPr / 10)).fill('').map((e, i) => <div onClick={() => dispatch(searchProducts({ query,page: i + 1 }))} key={Math.random()} className="px-2 py-1 text-sm text-blue-500 hover:dark:bg-slate-600 cursor-pointer hover:bg-blue-100/50- rounded-md dark:bg-slate-700 bg-blue-100/60" >
            {i + 1}
          </div>)}
        ...
        {new Array(searchnumOfPr && Math.ceil(searchnumOfPr / 10)).fill('') > 1 &&
          <div
            onClick={() => dispatch(searchProducts({query, page: searchPage + 1 }))}
            className="flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-slate-700"
          >
            <span>Next</span>
            <svg
